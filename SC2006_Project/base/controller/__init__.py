
#from SC2006_Project import settings
from .carparkController import *
from .contactController import *
from .faqController import *
from .findNearbyRestaurantController import *
from .homeController import *
from .loginController import *
from .restaurantController import *
from .reviewController import *
from .searchController import *
from .settingsController import *
from .signupController import *

